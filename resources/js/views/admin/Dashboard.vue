<template>
  <div class="flex h-screen bg-gray-50 font-sans text-gray-700 overflow-hidden">
    ...
    <div class="flex-1 flex flex-col h-screen overflow-hidden relative">
      <header
        class="h-20 bg-white/80 backdrop-blur-md border-b border-gray-100 flex items-center justify-between px-8 z-10 shrink-0"
      >
        <div class="flex items-center gap-4">
          <Button
            ...
            class="!w-10 !h-10 transition-transform active:scale-95"
          />
          <div>
        ...
        <div class="flex items-center gap-3">
          <span class="p-input-icon-left hidden sm:block">
            ...
            <InputText
              placeholder="Buscar..."
              class="!bg-gray-50 !border-gray-200 !rounded-full !py-2 !pl-10 !text-sm focus:!bg-white focus:!border-green-500 !w-64"
            />
          </span>
          <Button
            icon="pi pi-bell"
      ...
      <main class="flex-1 overflow-y-auto p-4 sm:p-8">
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          <div
            class="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex items-center gap-5 hover:-translate-y-1 transition-transform duration-300 group cursor-pointer"
          >
            <div
              class="w-14 h-14 rounded-2xl bg-blue-50 text-blue-600 group-hover:bg-blue-600 group-hover:text-white transition-colors flex items-center justify-center text-2xl shadow-sm"
            >
              <i class="pi pi-users"></i>
            </div>
            <div>
              <span class="block text-gray-400 text-xs font-bold uppercase tracking-wider"
                >Usuarios</span
              >
          ...
          >
            <div
              class="w-14 h-14 rounded-2xl bg-red-50 text-red-600 group-hover:bg-red-600 group-hover:text-white transition-colors flex items-center justify-center text-2xl shadow-sm"
            >
          ...
          >
            <div
              class="w-14 h-14 rounded-2xl bg-amber-50 text-amber-600 group-hover:bg-amber-600 group-hover:text-white transition-colors flex items-center justify-center text-2xl shadow-sm"
            >
              <i class="pi pi-tag"></i>
            </div>
            <div>
          ...
          >
            <div
              class="w-14 h-14 rounded-2xl bg-green-50 text-green-600 group-hover:bg-green-600 group-hover:text-white transition-colors flex items-center justify-center text-2xl shadow-sm"
            >
        ...
        <div
          class="bg-white border border-gray-100 rounded-3xl shadow-sm overflow-hidden flex flex-col h-auto animate-fade-in-up"
        >
          <div
            class="px-8 py-6 border-b border-gray-100 flex flex-col sm:flex-row justify-between items-center gap-4"
          >
            <div>
              <h3 class="text-xl font-bold text-gray-800">Directorio de Usuarios</h3>
              <p class="text-sm text-gray-400 mt-1">
                Gestión completa de accesos y roles
              </p>
            </div>
            <div class="flex gap-3 w-full sm:w-auto">
              <Button
                label="Nuevo Usuario"
                icon="pi pi-plus"
                class="!bg-green-600 !border-green-600 hover:!bg-green-700 !rounded-xl !px-6 !font-bold !shadow-lg hover:!shadow-xl transition-all"
      ...
      <form @submit.prevent="submitCreate" class="flex flex-col gap-6 pt-4">
        <FloatLabel>
          <InputText
            ...
            class="w-full !rounded-xl !py-3 !border-gray-300"
          />
        ...
        <FloatLabel>
          <Password
            ...
            :feedback="false"
            class="w-full"
            inputClass="w-full !rounded-xl !py-3 !border-gray-300"
          />
          <label for="n-pass">Contraseña</label>
        </FloatLabel>

        <div class="flex flex-col gap-2">
          <label class="text-xs font-bold text-gray-400 uppercase ml-2"
            >Asignar Rol</label
          >
          <Dropdown
        ...
        <div class="flex justify-end gap-3 pt-4 border-t border-gray-100 mt-2">
          ...
          <Button
            label="Crear Usuario"
            icon="pi pi-check"
            class="!bg-green-600 !border-green-600 !rounded-xl !px-6 !py-2.5 !font-bold !shadow-md hover:!shadow-lg hover:!-translate-y-0.5 transition-all"
            type="submit"
      ...
      <form @submit.prevent="submitEdit" class="flex flex-col gap-6 pt-4">
        ...
        <div class="flex justify-end gap-3 pt-4 border-t border-gray-100 mt-2">
          ...
          <Button
            ...
            icon="pi pi-save"
            class="!bg-blue-600 !border-blue-600 !rounded-xl !px-6 !py-2.5 !font-bold !shadow-md hover:!shadow-lg hover:!-translate-y-0.5 transition-all"
            type="submit"
    ...
    items: [
      {
        label: "Dashboard",
        icon: "pi pi-home",
        command: () => Inertia.visit("/admin/dashboard"),
...
const adminCount = computed(
  () => props.users.filter((u) => u.rol.toLowerCase().includes("admin")).length
);
...
const deleteUser = (user) => {
  if (confirm("¿Eliminar usuario?")) {
    Inertia.delete(`/admin/users/${user.id}`, {
      onSuccess: () =>
        toast.add({ severity: "info", summary: "Usuario Eliminado", life: 3000 }),
    });
  }
};
