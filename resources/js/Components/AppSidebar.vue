...
<template>
    ...
    >
      <!-- Logo Header -->
      <div class="h-20 flex items-center px-8 border-b border-gray-50 min-w-[18rem] shrink-0">
        <i class="pi pi-building text-2xl text-green-600 mr-2"></i>
        <span class="text-xl font-bold tracking-tight text-gray-800">Incidencias<span class="text-green-600">App</span></span>
        <!-- Close button always accessible -->
        <Button icon="pi pi-times" text rounded severity="secondary" class="ml-auto" @click="toggle" />
      </div>
      ...
      <div class="p-4 border-t border-gray-50 bg-gray-50/50 min-w-[18rem] shrink-0">
        <div class="flex items-center gap-3">
          <Avatar :label="user.name.charAt(0).toUpperCase()" shape="circle" class="!bg-white !text-green-700 shadow-sm !font-bold" />
          ...
          <Button icon="pi pi-sign-out" text rounded severity="secondary" @click="logout" v-tooltip="'Cerrar Sesión'" />
        </div>
      </div>
    </aside>
</template>
