<template>
  ...
  <div
    ...
    style="background: linear-gradient(180deg, rgba(255,255,255,0.02), rgba(13,18,23,0.18)); backdrop-filter: blur(6px) saturate(120%); -webkit-backdrop-filter: blur(6px) saturate(120%);"
    @click="toggle"
  ></div>
  ...
  <aside
    ...
    style="background-image: linear-gradient(170deg, rgba(255,255,255,0.2) 0%, rgba(255,255,255,0) 50%, rgba(255,255,255,0.05) 100%);"
    :class="[
  ...
  >
    ...
    <div class="h-36 flex items-center shrink-0 overflow-hidden relative" :class="isHovered || visible ? 'px-4' : 'justify-center'">
      <div class="flex items-center gap-3 transition-all duration-500" :class="isHovered || visible ? 'min-w-[280px]' : 'min-w-0'">
        <div class="w-20 h-20 flex items-center justify-center shrink-0 transition-all duration-1000 group-hover/sidebar:rotate-[360deg] group-hover/sidebar:scale-110">
          <img src="/public/img/jordan.png" alt="Jordan Logo" class="w-18 h-18 object-contain" />
        </div>
        <div class="flex flex-col transition-all duration-500" :class="isHovered || visible ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-10 pointer-events-none w-0 h-0 overflow-hidden'">
          <span class="text-3xl font-black tracking-tighter text-[#1D1D1F] leading-none whitespace-nowrap">Via<span class="text-[#607C88]">App</span></span>
          <span class="text-[11px] font-bold text-[#86868B] uppercase tracking-[0.1em] mt-2 whitespace-nowrap">Premium Management</span>
        </div>
      </div>
    ...
    <div class="flex-1 py-6 px-3 overflow-y-auto overflow-x-hidden custom-scrollbar">
      <nav class="space-y-10">
        <div v-for="(item, index) in menuItems" :key="index">
          ...
          <div 
            v-if="item.label" 
            class="px-5 mb-4 text-[11px] font-black text-[#86868B]/50 uppercase tracking-[0.2em] transition-all duration-500"
          ...
          <div class="space-y-2">
            <router-link 
              ...
              :key="subItem.label"
              :to="subItem.to || '#'"
              class="flex items-center h-12 px-3 rounded-[1.2rem] transition-all duration-500 group/item relative overflow-hidden"
              :class="$page.url === subItem.to 
                ? 'bg-[#1D1D1F] text-white shadow-xl shadow-[#1D1D1F]/20' 
            ...
            >
              ...
              <div class="w-10 flex justify-center items-center shrink-0">
                <i :class="[
                  ...
                  $page.url === subItem.to ? 'text-[#607C88]' : 'group-hover/item:text-[#607C88]'
                ]"></i>
              </div>
              ...
              <span 
                ...
                :class="isHovered || visible ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-8 pointer-events-none'"
              >
                {{ subItem.label }}
              </span>
              ...
              <div 
                v-if="$page.url === subItem.to && (isHovered || visible)"
                class="absolute right-4 w-1.5 h-1.5 bg-[#607C88] rounded-full shadow-[0_0_8px_#607C88]"
              ></div>
              
              <!-- Tooltip Minimalista (Negro Premium) -->
              <div 
                v-if="!isHovered && !visible"
                class="absolute left-20 bg-[#1D1D1F] text-white text-[10px] font-bold px-3 py-1.5 rounded-lg opacity-0 translate-x-4 group-hover/item:opacity-100 group-hover/item:translate-x-0 transition-all duration-300 pointer-events-none whitespace-nowrap z-50 shadow-2xl"
              >
                {{ subItem.label }}
              </div>
            </router-link>
    ...
    <div class="p-4 mt-auto">
      ...
      >
        <div class="relative shrink-0">
          ...
          />
          <div class="absolute -bottom-0.5 -right-0.5 w-3.5 h-3.5 bg-green-500 border-2 border-white/20 rounded-full"></div>
        </div>
        ...
        >
          <h4 class="text-sm font-bold text-[#1D1D1F] truncate tracking-tight">{{ displayUser.name }}</h4>
          <p class="text-[10px] text-[#86868B] font-bold uppercase tracking-tight truncate">{{ displayUser.rol }}</p>
        </div>
...
watch(() => props.visible, (val) => setBodyLock(!!val))
