  ...
  <div :class="[gridClass, gap, mb, 'grid']">
    ...
    >
      ...
      <div class="relative z-10">
        <span class="block text-[#86868B] dark:text-[#A1A1A6] text-[10px] font-black uppercase tracking-[0.2em] group-hover:text-white/70 transition-colors">
          {{ card.label }}
        </span>
        <span class="block text-4xl font-black text-[#1D1D1F] dark:text-white tracking-tighter mt-1 group-hover:text-white transition-all transform origin-left">
          {{ card.value }}
        </span>
      </div>
    </div>
...
const AUTO_STYLES = [
  { keys: ['pendiente'],           icon: 'pi pi-clock',        bg: 'bg-amber-50 dark:bg-amber-500/10',   text: 'text-amber-500',   hover: 'hover:bg-[#D97706] hover:shadow-[0_20px_50px_rgba(217,119,6,.3)] hover:border-[#D97706]' },
  { keys: ['proceso', 'progreso'], icon: 'pi pi-sync',         bg: 'bg-sky-50 dark:bg-sky-500/10',     text: 'text-sky-600 dark:text-sky-400',     hover: 'hover:bg-[#0369A1] hover:shadow-[0_20px_50px_rgba(3,105,161,.3)] hover:border-[#0369A1]' },
  { keys: ['resuelto','completado','done','listo'], icon: 'pi pi-check-circle', bg: 'bg-emerald-50 dark:bg-emerald-500/10', text: 'text-emerald-600 dark:text-emerald-400', hover: 'hover:bg-[#059669] hover:shadow-[0_20px_50px_rgba(5,150,105,.3)] hover:border-[#059669]' },
  { keys: ['rechazado','cancelado','error'],        icon: 'pi pi-times-circle', bg: 'bg-rose-50 dark:bg-rose-500/10',    text: 'text-rose-600 dark:text-rose-400',    hover: 'hover:bg-[#DC2626] hover:shadow-[0_20px_50px_rgba(220,38,38,.3)] hover:border-[#DC2626]' },
  { keys: ['activo','active'],     icon: 'pi pi-check-circle', bg: 'bg-green-50 dark:bg-green-500/10',   text: 'text-green-600 dark:text-green-400',   hover: 'hover:bg-[#34C759] hover:shadow-[0_20px_50px_rgba(52,199,89,.3)] hover:border-[#34C759]' },
  { keys: ['admin'],               icon: 'pi pi-shield',       bg: 'bg-[#F5F5F7] dark:bg-white/5', text: 'text-[#607C88] dark:text-gray-400',   hover: 'hover:bg-[#607C88] hover:shadow-[0_20px_50px_rgba(96,124,136,.3)] hover:border-[#607C88]' },
]
...
function styleForValue(val, fallbackIdx = 0) {
  ...
  return { icon: 'pi pi-tag', bgClass: 'bg-[#F5F5F7] dark:bg-white/5', textClass: 'text-[#1D1D1F] dark:text-white', hoverColor: fb.hover }
}
...
const resolvedStats = computed(() => {
  ...
  const result = [{
    ...
    bgClass:    'bg-[#F5F5F7] dark:bg-white/5',
    textClass:  'text-[#1D1D1F] dark:text-white',
